...
object Samples {
    ...
    init {
        ...
        val videosUrl = "http://192.168.0.22:8080/testjs.json"
        //Audio items
...
*/
        ...
        try {
            val jsonArray = JSONArray(URL(videosUrl).readText())
            for (jsonIndex in 0 until jsonArray.length()) {
                var title = jsonArray.getJSONObject(jsonIndex).getString("title").toString();
                var url = jsonArray.getJSONObject(jsonIndex).getString("url").toString();
                var videoImage = jsonArray.getJSONObject(jsonIndex).getString("videoImage").toString();
                videoSamples.add(Sample(title,url, videoImage))
            }
        } catch(e: Exception){
            Log.e("Error calling function:", e.toString());
        }
    }
